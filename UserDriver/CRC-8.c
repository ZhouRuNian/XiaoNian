
 #include "CRC-8.h"
const unsigned char CRC_8_TAB[] = {
	
};
unsigned char Get_Crc8_LookUp(const volatile unsigned char *pData, unsigned short nLen, unsigned char Crc, const unsigned char *pCrc_Tab)
{
	volatile unsigned char Crc_8 = Crc;
	while(nLen>0)
	{
			Crc_8 = pCrc_Tab[Crc_8^*pData];
			nLen--;
			pData++;
	}
	return Crc_8;    
}
